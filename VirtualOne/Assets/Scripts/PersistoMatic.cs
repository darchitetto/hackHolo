...
public class PersistoMatic : MonoBehaviour
{
    ...
    [Tooltip("Distance in front of camera when placing.")]
    public float CameraDistance = .02f;

    [Tooltip("Unique name for the Anchor.")]
    ...
    void Update()
    {
        if (Placing)
        {
            gameObject.transform.position = Camera.main.transform.position + Camera.main.transform.forward * CameraDistance;
        }
    }
